...
class LMODdb(object):
  ...
  def counts(self, sqlPattern, syshost, startDate, endDate, allhosts, modulename):
    query = ""
    try:
      conn   = self.connect()
      cursor = conn.cursor()
      ...
      if (modulename == "unknown"):
        selpath = "t1.path"
        sellabel = "Module path"
      else:
        selpath = "t1.module"
        sellabel = "Module name"

      if (allhosts == "unknown"):
        query = "SELECT " + selpath + ", t1.syshost, count(distinct(t2.user_id)) as c3 from moduleT as t1, " +\
        ...
        query = "SELECT " + selpath + ", count(distinct(t2.user_id)) as c3 from moduleT as t1, "  +\
                "join_user_module as t2 where t1.path like %s and t1.mod_id = t2.mod_id " +\
                "and t1.syshost like %s " + dateTest + " group by " + selpath + " order by c3 desc"
      ...
      if (allhosts == "unknown"):
        resultA.append([sellabel, "Syshost", "Distinct Users" ])
        resultA.append(["-----------", "-------", "--------------"])
      else:
        resultA.append([sellabel, "Distinct Users" ])
        resultA.append(["-----------", "--------------"])
      for i in xrange(numRows):
        row = cursor.fetchone()
        if (allhosts == "unknown"):
  ...
  def usernames(self, sqlPattern, syshost, startDate, endDate,modulename):
    query = ""
    try:
      conn   = self.connect()
      cursor = conn.cursor()
      ...
      query =  "SELECT " + selpath + ", t3.user as c2, count(distinct(t1.syshost)) as c3 from moduleT as t1, join_user_module "       +\
      ...
      resultA = []
      resultA.append([sellabel, "User Name", "Syshost count"])
      resultA.append(["-----------", "-------", "---------"])
  ...
  def modules_used_by(self, username, syshost, startDate, endDate, modulename):
    query = ""
    try:
      conn   = self.connect()
      cursor = conn.cursor()
      ...
      query =  "SELECT " + selpath + " as c1, t3.user as c2, count(distinct(t1.syshost)) as c3 from moduleT as t1, join_user_module "   +\
